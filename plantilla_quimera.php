...
<head>
    ...
    <style>
        /* ---
        SISTEMA DE DISEÑO QUIMERA v1.2 (Corrección Definitiva)
        --- */

        /* 1. VARIABLES Y TEMAS (Sin cambios) */
        :root {
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        #aurora-background { position: fixed; top: 0; left: 0; width: 100%; height: 100%; z-index: -2; transition: opacity var(--transition-slow) ease; opacity: var(--aurora-opacity); }
        ...
        /* 3. ESTRUCTURA PRINCIPAL (Sin cambios) */
        ...
        /* 4. NAVEGACIÓN QUIMERA (REFACTORIZADA) */
        #main-nav {
        ...
        .nav-link {
            ...
            width: 100%;
            height: 56px; /* FIX: Altura fija para alineación perfecta */
            color: var(--c-text-secondary); text-decoration: none;
            font-weight: 600; font-size: 1rem;
            position: relative;
            padding: 0; /* FIX: Reseteamos padding para control total */
        }
        body.nav-expanded .nav-link {
            justify-content: flex-start;
            padding: 0 32px; /* Añadimos padding solo cuando está expandido */
        }
        
        .nav-link svg {
            width: 24px; height: 24px; stroke: var(--c-text-secondary); flex-shrink: 0;
            transition: stroke var(--transition-fast) ease;
        }
        
        /* FIX: La solución definitiva para ocultar el texto */
        .nav-link-text {
            position: absolute; /* Lo sacamos del flujo normal */
            left: 72px; /* Posición fija cuando está expandido */
            opacity: 0;
            transform: translateX(-10px);
            transition: opacity var(--transition-medium) ease, transform var(--transition-medium) ease;
            pointer-events: none; /* Evita que se pueda interactuar con el texto oculto */
        }
        body.nav-expanded .nav-link-text {
            position: static; /* Vuelve al flujo normal */
            opacity: 1;
            transform: translateX(0);
            pointer-events: auto;
            margin-left: 20px; /* Espacio entre icono y texto */
        }
        
        .nav-link.active, .nav-link:hover { color: var(--c-text); }
        .nav-link.active svg, .nav-link:hover svg { stroke: var(--c-accent); }
        
        /* FIX: Indicador de link activo (la barra vertical) */
        .nav-link::before {
            ...
            display: none; /* FIX: Oculto por defecto */
        }
        body.nav-expanded .nav-link::before {
            display: block; /* FIX: Visible solo en estado expandido */
            left: 10px;
        }
        ...
        /* FIX: Aseguramos que el botón de menú sea un nav-link más */
        #nav-toggle {
            background: none; border: none; cursor: pointer;
        }

        /* 6. CAMBIADOR DE TEMA (REVISADO) */
        #theme-switcher {
            display: flex; justify-content: center; gap: 8px; 
            height: 56px; /* FIX: Misma altura que los nav-links */
            align-items: center;
        }
        body.nav-expanded #theme-switcher {
            justify-content: flex-start;
            padding: 0 32px;
        }
